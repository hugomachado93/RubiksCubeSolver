...
*     12      dlas   02/mai/2018 Adição das assertivas de entrada e saída
*     11	  dlas   02/mai/2018 Mudança na assinatura da função de criação
...
/* Vetor com configuração de cubo */

typedef struct CUB_config {
	int cores[54];
} CUB_tpConfiguracao;
...
typedef enum {
	...
	CUB_CondRetConfiguracaoInvalida
	/* A configuração inicial do cubo não contém 6 cores 
	diferentes com 9 ocorrências cada */
...
*		- A configuração inicial contém 6 (seis) cores e cada cor tem 
*		exatamente 9 (nove) ocorrências.
*
*	$AS Assertivas de Saída
*		- Cubo alocado na memória com as faces contendo as cores descritas 
*		pelo parâmetro de configuração inicial.
*		- Valem as assertivas estruturais do cubo.
*
*  $FV Valor retornado
*		CUB_CondRetOK					- Criou um cubo com sucesso.
*		CUB_CondRetFaltouMemoria		- Erro ao criar o cubo por falta de
*										  memória.
*		CUB_CondRetConfiguracaoInvalida	- Vetor de configuração inicial
*										  fornecido não tem número de
*										  cores consistente
*
**************************************************************************/

CUB_tpCondRet CUB_CriarCUBO(CUB_tppCUBO* pCubo, CUB_tpConfiguracao ConfiguracaoInicial);
...
*
*	$AE Assertivas de Entrada
*		- O endereço recebido aponta para uma estrutura válida de cubo.
*
*	$AS Assertivas de Saída
*		- A configuração das cores é a mesma inicialmente fornecida.
*
*  $FV Valor retornado
...
*
*	$AS Assertivas de Saída
*		- A memória ocupada pelo cubo inicialmente provida está desalocada.
*		
*  $FV Valor retornado
...
*	$AE Assertivas de Entrada
*		- O endereço recebido aponta para uma estrutura válida de cubo.
*		- A face fornecida é um valor válido, ou seja, descreve alguma 
*		orientação espacial conforme descrito pelo tipo enumerado 
*		"CUB_tpFaceDeCUBO".
*
*	$AS Assertivas de Saída
*		- A configuração das cores agora descreve um cubo girado (na face 
*		descrita) um número "numGiros" de vezes no sentido fornecido.
*		- Valem as assertivas estruturais do cubo.
*
*  $FV Valor retornado
*	CUB_CondRetOK			-	Executou normalmente.
...
*	$AE Assertivas de Entrada
*		- O endereço recebido aponta para uma estrutura válida de cubo.
*		- Os valores de linha e coluna descrevem posições sobre faces.
*		- A face fornecida é um valor válido, ou seja, descreve alguma
*		orientação espacial conforme descrito pelo tipo enumerado
*		"CUB_tpFaceDeCUBO".
*
*	$AS Assertivas de Saída
*		- A configuração das cores é a mesma inicialmente fornecida.
*		- O conteúdo de pCor contém a cor do quadrado descrito pelos 
*		demais parâmetros.
*
*  $FV Valor retornado
*	CUB_CondRetOK					- Executou normalmente.
...
*	$AE Assertivas de Entrada
*		- O endereço recebido aponta para uma estrutura válida de cubo.
*		- As cores fornecidas estão inseridas dentro do cubo.
*
*	$AS Assertivas de Saída
*		- A configuração das cores é a mesma inicialmente fornecida.
*		- O conteúdo de pFace, pLinha e pColuna descrevem a peça encontrada.
*
*  $FV Valor retornado
*	CUB_CondRetOK			-	Executou normalmente.
*	CUB_CondRetCuboInvalido	-	Recebeu um ponteiro NULL.
...
/********** Fim do módulo de definição: CUB  Cubo **********/
