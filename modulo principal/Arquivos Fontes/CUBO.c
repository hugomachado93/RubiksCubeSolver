...
static void zeraVetor(int* vetor, int tam); //Documentado dentro da função
...
CUB_tpCondRet CUB_CriarCUBO(CUB_tppCUBO* pCubo, CUB_tpConfiguracao ConfiguracaoInicial) //Função para criação do cubo
{
...
void imprimirElemento(CUB_tagCUBO* cubo, int x, int y, int z, CUB_tpFaceDeCUBO face) //Função que imprime o elemento especificado do cubo
{
	...
	CUB_tpPecadeCubo unidadeDeCubo = cubo->estrutura[indice];
...
void funcaoDebugger(CUB_tagCUBO* cubo) //Função de exibição teste para o cubo
{
  ...
  /***************************************************************************
  *
  *  Funcão: CUB Checa consistência da configuração passada
  *
  ***************************************************************************/
	...
	for (i = 0; i < NUMERODEFACES*UNIDADESPOREIXO*UNIDADESPOREIXO; i++)
	{
		...
		if (pos < 0) // Cor nao encontrada no vetor nesse momento
		{
			if (numDeCoresEncontradas >= NUMERODEFACES)
				return 0; // O cubo contém mais do que 6 cores
		...
		else
		{
			...
			if (ocorrencias[pos] > UNIDADESPOREIXO*UNIDADESPOREIXO)
				return 0; // A cor possui mais de 9 ocorrências
		}
	}
	return 1;
} //Funcao que checa se o vetor está com 6 cores de 9 ocorrências cada

/***************************************************************************
*
*  Funcão: Retorna o índice da primeira ocorrência de "chave" em "vetor"
...
/***************************************************************************
*
*  Funcão: Atribui valor zero a todas posições de um vetor de tamanho 'tam'
*
***************************************************************************/
